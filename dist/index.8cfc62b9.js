...
var _vocabularyJs = require("./vocabulary.js");
var _vocabularyJsDefault = parcelHelpers.interopDefault(_vocabularyJs);
'use strict';
window.onload = function() {
    ...
    const updateWords = function() {
        ...
        wordsArray.forEach((word, i)=>{
            ...
            if (wordsArray.length < 10) document.getElementById(`word-${wordsArray.length + 1}`).textContent = '';
        });
    };
    ...
    let wrongNbr = 0;
    let japToKor = true;
    let wordIndex = _vocabularyJsDefault.default.indexOf(wordsArray[0]);
    let randomWord = wordsArray[0];
    let answerKor = _vocabularyJs.koreanWords[wordIndex];
    let answerJpy = _vocabularyJsDefault.default[wordIndex];
    const randomPolWord = _vocabularyJs.polishWords[wordIndex];
    const randomEngWord = _vocabularyJs.englishWords[wordIndex];
    ...
    if (displayedWord.textContent.length > 9) displayedWord.style.fontSize = `21px`;
    else displayedWord.style.fontSize = `35px`;
    ...
    const hideShowlang = function(selectedSwitch, word) {
        document.getElementById(`${selectedSwitch}`).addEventListener("click", function() {
            ...
            else document.getElementById(`${word}`).style.display = "grid";
        });
    };
    const newWord = function() {
        newRandomNumber = Math.trunc(Math.random() * wordsArray.length);
        randomWord = wordsArray[newRandomNumber];
        wordIndex = japToKor ? _vocabularyJsDefault.default.indexOf(randomWord) : _vocabularyJs.koreanWords.indexOf(randomWord);
        document.getElementById('current-display-word').textContent = randomWord;
        if (displayedWord.textContent.length > 9) displayedWord.style.fontSize = `21px`;
        else displayedWord.style.fontSize = `35px`;
        console.log(displayedWord.style.fontSize);
        japToKor ? answerKor = _vocabularyJs.koreanWords[wordIndex] : answerJpy = _vocabularyJsDefault.default[wordIndex];
        document.getElementById('polishWord').textContent = _vocabularyJs.polishWords[wordIndex];
        document.getElementById('englishWord').textContent = _vocabularyJs.englishWords[wordIndex];
    ...
    const checkAnswer = function() {
        ...
        if (japToKor) {
            if (input === answerKor || answerKor.includes(input) && input !== "") {
                ...
                document.getElementById('feedback').textContent = `wrong answer! 😢 Correct answer was: \u00A0 \u00A0
                   ` + `  ${answerKor}`;
                wrongAnswer();
            }
        }
        if (!japToKor) {
            if (input === answerJpy || answerJpy.includes(input) && input !== "") {
                ...
                updateWords();
                correct();
                if (wordsArray.length === 0) newWordsArray(_vocabularyJs.koreanWords);
            } else {
                document.getElementById('feedback').textContent = `wrong answer! 😢 Correct answer was: \u00A0 \u00A0
                     ${answerJpy}`;
                wrongAnswer();
